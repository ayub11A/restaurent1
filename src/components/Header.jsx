...
function Header() {
  ...
  const { cart } = useCart();   // <-- cart sax ah
  const { isAdmin } = useAdmin();

  // FIXED: prevent undefined reduce error
    ...
    <header className="fixed top-0 left-0 w-full bg-gray-700 shadow-md z-50">
      <nav className="flex justify-between items-center px-6 py-3 md:py-4">
        ...
        <div className="flex items-center gap-2">
          ...
          />
          <h1 className="text-green-500 text-2xl -mt-5">
            <span className="text-white">
              <span className="text-amber-400">K</span>
              <span className="text-red-800">C</span>
            </span>
          </h1>
          <h2 className="mt-5 -ml-7">
            <span className="text-red-500">chicken</span>
            <span className="text-black">&</span>
            <span className="text-amber-200">chips</span>
          </h2>
        </div>
